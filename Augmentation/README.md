...
CMcriterion = CutMixCriterion(criterion, device='cuda') # criterion when the probabiliry over the 0.75
...
for e in range(0, n_epochs):
    for data, labels in tqdm(train_loader):
        rand_num = np.random.rand() # Create the Random Numver between 0~1, then it will be probability

        if rand_num > 0.75: data, label= cutmix(data, labels) # Using the Cutmix if probability is bigger than 0.75 
        ...
        data = data.to(device) # labels.to(device) # move tensors to GPU if CUDA is available     
